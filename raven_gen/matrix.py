...
from .attribute import AttributeType, PlanarPosition, PositionType
from .component import AttributeHistory, ComponentType, LayoutType, make_component
...
class Matrix:
    ...
    attribute_bounds = {
        MatrixType.ONE_SHAPE: {(ComponentType.NONE, LayoutType.CENTER): {}},
        MatrixType.FOUR_SHAPE: {(ComponentType.NONE, LayoutType.GRID_FOUR): {}},
        MatrixType.FIVE_SHAPE: {(ComponentType.NONE, LayoutType.GRID_FIVE): {}},
        MatrixType.NINE_SHAPE: {(ComponentType.NONE, LayoutType.GRID_NINE): {}},
    ...
    @classmethod
    def make(
        cls, matrix_type: MatrixType, ruleset: Ruleset = None, n_alternatives: int = 0
    ) -> "Matrix":
    ...
    @staticmethod
    def validate_bounds(src, dst, attribute: AttributeType, min_setting, max_setting):
    ...
    def get_attribute_bounds(cls, matrix_type: MatrixType):
        validated = {}
        for (component_type, layout_type), src in cls.attribute_bounds[
            matrix_type
        ].items():
            assert component_type in ComponentType and layout_type in LayoutType
            dst = {}
            ...
            cls.validate_bounds(
                src, dst, AttributeType.NUMBER, attribute.NUM_MIN, number_max
            )
            cls.validate_bounds(
                src,
                dst,
                AttributeType.SHAPE,
                attribute.SHAPE_MIN + 1,
                attribute.SHAPE_MAX,
            )
            if component_type is ComponentType.OUT:
                size_min, color_max = attribute.SIZE_MIN + 3, attribute.COLOR_MIN
            elif (
                component_type is ComponentType.IN
                and layout_type is LayoutType.GRID_FOUR
            ):
                size_min = attribute.SIZE_MIN + 2
            else:
                size_min, color_max = attribute.SIZE_MIN, attribute.COLOR_MAX
            cls.validate_bounds(
                src, dst, AttributeType.SIZE, size_min, attribute.SIZE_MAX
            )
            cls.validate_bounds(
                src, dst, AttributeType.COLOR, attribute.COLOR_MIN, color_max
            )
            cls.validate_bounds(
                src, dst, AttributeType.ANGLE, attribute.ANGLE_MIN, attribute.ANGLE_MAX
            )
            cls.validate_bounds(
                src, dst, AttributeType.UNIFORMITY, attribute.UNI_MIN, attribute.UNI_MAX
            )
    ...
    def four_shape(cls, bounds, ruleset, n_alternatives):
        return cls(
            ...
            Panel(
                make_component(
                    ComponentType.NONE,
                    LayoutType.GRID_FOUR,
                    PositionType.PLANAR,
                    [
                        PlanarPosition(x_c=0.25, y_c=0.25, max_w=0.5, max_h=0.5),
                        PlanarPosition(x_c=0.25, y_c=0.75, max_w=0.5, max_h=0.5),
                        PlanarPosition(x_c=0.75, y_c=0.25, max_w=0.5, max_h=0.5),
                        PlanarPosition(x_c=0.75, y_c=0.75, max_w=0.5, max_h=0.5),
                    ],
    ...
    def five_shape(cls, bounds, ruleset, n_alternatives):
        return cls(
            ...
            Panel(
                make_component(
                    ComponentType.NONE,
                    LayoutType.GRID_FIVE,
                    PositionType.PLANAR,
                    [
                        PlanarPosition(x_c=0.2, y_c=0.2, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.2, y_c=0.8, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.5, y_c=0.5, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.8, y_c=0.2, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.8, y_c=0.8, max_w=0.33, max_h=0.33),
                    ],
    ...
    def nine_shape(cls, bounds, ruleset, n_alternatives):
        return cls(
            ...
            Panel(
                make_component(
                    ComponentType.NONE,
                    LayoutType.GRID_NINE,
                    PositionType.PLANAR,
                    [
                        PlanarPosition(x_c=0.16, y_c=0.16, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.16, y_c=0.5, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.16, y_c=0.83, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.5, y_c=0.16, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.5, y_c=0.5, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.5, y_c=0.83, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.83, y_c=0.16, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.83, y_c=0.5, max_w=0.33, max_h=0.33),
                        PlanarPosition(x_c=0.83, y_c=0.83, max_w=0.33, max_h=0.33),
                    ],
    ...
    def make_ground_truth(self, base, ruleset):
        ...
        while True:
            self.rules = ruleset.sample(
                tuple((c.component_type, c.layout_type) for c in base.components)
            )
            pruned = prune(base, self.rules)
            self.start = copy.deepcopy(pruned) if pruned is not None else pruned
    ...
    def make_row(self):
        ...
        for comp in col_0.components:
            comp.sample(sample_position=True, sample_number=True, carryover=False)
        comp_rows = []
    ...
    def make_alternatives(self, n_alternatives):
        ...
        if n_alternatives > 0:
            ...
            self.uniques = [AttributeHistory(cst) for cst in self.initial_constraints]
    ...
    def count_modifiable(self) -> List[list]:
        ...
        for c, (component, component_rules, initial_constraints) in enumerate(
            zip(self.answer.components, self.rules, self.initial_constraints)
        ):
            ...
            for n_entities in range(
                ...
                component.config.number.value_of_setting(initial_constraints.number.max)
                + 1,
            ):
            ...
            if component_rules.configuration.attr is not AttributeType.NUMBER:
                position_samples = comb(max_entities, component.config.number.value) - 1
                if position_samples > 0:
                    samples.append([c, AttributeType.POSITION, position_samples])
            for rule in component_rules:
                if (
                    rule.name is not RuleType.CONSTANT
                    or component.uniformity.value
                    or component_rules.configuration.name is RuleType.CONSTANT
                    or (
                        component_rules.configuration.attr is AttributeType.POSITION
                        and component_rules.configuration.name
                        is not RuleType.ARITHMETIC
                    )
                ):
                    bounds = getattr(initial_constraints, rule.attr.name.lower())
    ...
    def render(
        ...
        panel_img = np.ones((panel_size, panel_size), np.uint8) * background_color
        for entity in entities:
            entity_img = entity.render(
                background_color, panel_size, shape_border_thickness
            )
    ...
    def generate_matrix(
        ...
        img_grid = np.ones((image_size, image_size), np.uint8) * background_color
        for pos, panel in enumerate(
            ...
            i, j = divmod(pos, 3)
            img_grid[
                i * panel_size : (i + 1) * panel_size,
                j * panel_size : (j + 1) * panel_size,
            ] = panel
        ...
        if self.structure_type is StructureType.LEFT_RIGHT:
            for i in range(3):
                ...
                img_grid[:, band_center - neg_offset : band_center + pos_offset] = 0.0
    ...
    def save(
        ...
        image_size, background_color, line_thickness, shape_border_thickness = (
            int(abs(image_size)),
            int(abs(background_color)),
            int(abs(line_thickness)),
            int(abs(shape_border_thickness)),
        )
        assert image_size != 0 and background_color <= 255

        # Calculate the position of the bottom-right cell
        cell_size = image_size // 3
        x0, y0 = 2 * (cell_size + line_thickness), 2 * (
            cell_size + line_thickness
        )  # Third row, third column (0-based index 2,2)

        # Save the answer image
        img = self.generate_matrix(
            self.answer,
            background_color,
            image_size,
            line_thickness,
        ...
        img = img.crop(
            (
                x0,
                y0,
                x0 + cell_size - 3 * line_thickness,
                y0 + cell_size - 3 * line_thickness,
            )
        )
