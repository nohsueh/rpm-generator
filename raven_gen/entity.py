...
class Entity:
    ...
    def render(self, background_color, panel_size, shape_border_thickness):
        ...
        center = Point(
            y=int(self.bbox.y_c * panel_size), x=int(self.bbox.x_c * panel_size)
        )
        ...
        if isinstance(self.bbox, AngularPosition):
            return rotate(
                ...
                panel_size,
                center=Point(
                    x=(self.bbox.x_r * panel_size), y=(self.bbox.y_r * panel_size)
                ),
            )
        elif isinstance(self.bbox, PlanarPosition):
            return rotate(
                img, self.angle.value, background_color, panel_size, center=center
            )
        else:
